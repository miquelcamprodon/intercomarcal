...
def ranquing_primera_fase(request,comp_nom,l_comp_1f,precalcula=False):        
    ...
    if precalcula:
        ...
        from django.db import connectionc
...
def ranquingtti(request):
    ...
    query=''' SELECT *
                FROM programa_tornejosranquingtti'''
                
    cursor.execute(query)
  
    ttornejostti = cursor.fetchall()       
    
    #No passem els dos primers elements dels tornejos perque son els camps jugador i club que ja hi son ni tampoc lultim que es el total
    #Es per un tema de format a la plantilla ranquingtti.html
    
    return render_to_response('programa/ranquingtti.html',
            {'ranquingtti':tranquingtti,'tornejostti':ttornejostti[2:len(ttornejostti)-1]},
            context_instance=RequestContext(request))
