...
from intercomarcal36.programa.forms import ActaForm,LoginForm,ActaFormCD
...
def calendari(request,comp_nom):
    ...
    return render(request, 'programa/calendari.html',
        {'competicio':comp_nom,'ljordesc':ljordesc,'equip_descansa':(len(ldescansa) > 0)}) 
...
def calendari_entrar(request,comp_nom):
    ...
    return render(request,'programa/calendari_entrar.html',
        {'competicio':comp_nom,'ljordesc':ljordesc,'equip_descansa':(len(ldescansa) > 0)})
...
def classificacio(request,comp_nom):
    ...
    elif comp_nom == "20162f1" or comp_nom == "20162f2":
        return classificacio_2_lligueta(request,comp_nom)
    # 2015    
    if comp_nom == "20151f":
        return classificacio_1_lligueta(request,comp_nom)
    elif comp_nom == "20152f1" or comp_nom == "20152f2":
        return classificacio_2_lligueta(request,comp_nom)
    # 2014    
    if comp_nom == "20141f":
        return classificacio_1_lligueta(request,comp_nom)
    elif comp_nom == "20142f1" or comp_nom == "20142f2" or comp_nom == "20142f3":
        return classificacio_2_lligueta(request,comp_nom)
    # 2013
    elif comp_nom == "2013g1" or comp_nom == "2013g2":
        return classificacio_1_lligueta(request,comp_nom)
    # 2012    
    ...
    # 2011    
        ...
        return classificacio_2_lligueta(request,comp_nom)
    # 2010
    elif comp_nom == "20101fA" or comp_nom == "20101fB":
        return classificacio_1_lligueta(request,comp_nom)
...
def classificacio_1_lligueta(request,comp_nom):
    ...
    for e in equips:
        ...
        lequips[e]['pe']=0 #partits empatats
        lequips[e]['dg']=0 #dobles guanyats
        lequips[e]['dp']=0 #dobles perduts
        # Local games
        enclist = enccomp.filter(local__equip=e.equip)
        for enc in enclist:
            if enc.jocs_local != None:
                ...
                if (comp_nom == "20151f" or comp_nom == "20161f"):
                    ...
                    if (enc.guanyador_doble == 1):
                        ...
                        lequips[e]['dg'] = lequips[e]['dg'] + 1                        
                    else:
                        lequips[e]['dp'] = lequips[e]['dp'] + 1  
                    if (enc.resultat_local_sense_doble >= 4):
                        lequips[e]['punts'] = lequips[e]['punts'] + 2
                    elif (enc.resultat_local_sense_doble == 3):
                        lequips[e]['punts'] = lequips[e]['punts'] + 1                        
                    else:
                        lequips[e]['punts'] = lequips[e]['punts'] + 0              
                else:
    ...
    # Ordenem els equips entre ells (si n hi ha)
    if len(loteams) > 0:
    ...
    tclass = []
    for e in loteams:
        tclass.append({"equipcompeticio":e[0],"pj":e[1]['pj'],"pg":e[1]['pg'],"pe":e[1]['pe'],"pp":e[1]['pp'],"dg":e[1]['dg'],"dp":e[1]['dp'],"puf":e[1]['puf'],"puc":e[1]['puc'],"sf":e[1]['sf'],"sc":e[1]['sc'],"punts":e[1]['punts']})

    return render(request,'programa/classificacio.html',
        {'classificacio':tclass, 'competicio':comp_nom})
...
def classificacio_2_lligueta(request,comp_nom):
    ...
    dic_1_lligueta['20142f1'] = ['20141f']
    dic_1_lligueta['20142f2'] = ['20141f']
    dic_1_lligueta['20142f3'] = ['20141f']    

    dic_1_lligueta['20152f1'] = ['20151f']
    dic_1_lligueta['20152f2'] = ['20151f']        

    dic_1_lligueta['20162f1'] = ['20161f']
    dic_1_lligueta['20162f2'] = ['20161f']  
        ...
        for enc in enclist:
            if enc.jocs_local != None:
                ...
                if (enc.resultat_local >= 4):
                    ...
                    lequips[e]['pj'] = lequips[e]['pj'] + 1
                    lequips[e]['pp'] = lequips[e]['pp'] + 1
                if (comp_nom == "20152f1" or comp_nom == "20152f2" or comp_nom == "20162f1" or comp_nom == "20162f2"):
                    #El camp guanyador_doble es 1 pel local o 2 pel visitant
                    if (enc.guanyador_doble == 1):
                        lequips[e]['punts'] = lequips[e]['punts'] + 1
                        lequips[e]['dg'] = lequips[e]['dg'] + 1                        
...
def ranquing(request,comp_nom):
    """
    Per utilitzar el ranquingprecalcula hem de comentar i descomentar els returns de les competicions
    ...
    elif comp_nom == '20162f12':
        l_comp_12f = ['20161f','20162f1','20162f2']
        l_comp_2punts = ['20161f','20162f1']
        l_comp_1_5punts = []
        l_comp_1punt = ['20162f2']
    ...
    if comp_nom == '20151f':    
        l_comp_1f = ['20151f']
        return ranquing_primera_fase(request,comp_nom,l_comp_1f)
    elif comp_nom == '20152f12':
        l_comp_12f = ['20151f','20152f1','20152f2']
        l_comp_2punts = ['20151f','20152f1']
        l_comp_1_5punts = ['20152f2']
        l_comp_1punt = []
        #return ranquing_segona_fase(request,comp_nom,l_comp_12f,l_comp_2punts,l_comp_1punt) 
        return ranquing_segona_fase(request,comp_nom,l_comp_12f,l_comp_2punts,l_comp_1punt,l_comp_1_5punts)   
        #return ranquing_utilitzant_precalcula(request)
    ...
    if comp_nom == '20141f':    
        l_comp_1f = ['20141f']
        return ranquing_primera_fase(request,comp_nom,l_comp_1f)
    elif comp_nom == '20142f123':
        l_comp_12f = ['20141f','20142f1','20142f2','20142f3']
        l_comp_2punts = ['20141f','20142f1']
        l_comp_1_5punts = ['20142f2']
        l_comp_1punt = ['20142f3']
        #return ranquing_segona_fase(request,comp_nom,l_comp_12f,l_comp_2punts,l_comp_1punt) 
        return ranquing_segona_fase(request,comp_nom,l_comp_12f,l_comp_2punts,l_comp_1punt,l_comp_1_5punts)   
        #return ranquing_utilitzant_precalcula(request)

    if comp_nom == '2013g12':
        l_comp_12f = ['2013g1','2013g2']
        l_comp_2punts = ['2013g1']
        l_comp_1_5punts = ['2013g2']
        l_comp_1punt = []
        #return ranquing_segona_fase(request,comp_nom,l_comp_12f,l_comp_2punts,l_comp_1punt,l_comp_1_5punts)
        return ranquing_utilitzant_precalcula(request)
...
def ranquing_primera_fase(request,comp_nom,l_comp_1f,precalcula=False):        
    ...
    if precalcula:
        ...
        return render(request,'programa/ranquing.html',
...
def ranquing_precalcula():
    ...
    l_comp_1_5punts = ['20142f2']
    l_comp_1punt = ['20142f3']
    ranquing_segona_fase('','20142f123',l_comp_12f,l_comp_2punts,l_comp_1punt,l_comp_1_5punts,precalcula=True)
    ...
    l_comp_1_5punts = ['2013g2']
    l_comp_1punt = []
    ranquing_segona_fase('','2013g12',l_comp_12f,l_comp_2punts,l_comp_1punt,l_comp_1_5punts,precalcula=True)
...
def veure_acta(request,acta_id):   
    ...
    return render(request,'programa/veure_acta.html',
        {'myform':myform,'local':enc.local,'visitant':enc.visitant,'acta_id':acta_id, 'competicio': comp_nom})
...
def veure_acta_cd(request,acta_id):   
    ...
    for fieldname in myform.fields:
        myform.fields[fieldname].widget.attrs['readonly'] = True
        #myform.fields[fieldname].widget.attrs['editable'] = False
    return render(request,'programa/veure_acta_cd.html',
        {'myform':myform,'local':enc.local,'visitant':enc.visitant,'acta_id':acta_id, 'competicio': comp_nom})
    ...
    if request.method == 'POST':
        ...
        if myloginform.is_valid():
            ...
            if user is None:
                ...
                response = render(request,'programa/login_acta.html',
...
def acta(request,acta_id):
    ...
    if request.method == 'POST':
        ...
        if myform.is_valid():
            ...
            if err:
                return render(request,'programa/acta.html',
            ...
            if enc.abc == enc.local:
                ...
                # Omplim el camp guanyador del doble. Si es un 1 guanya el local si es un 2 el visitant.
                if fcd[lresabc[3]] == 3:
                    enc.guanyador_doble = 1
                    enc.resultat_local_sense_doble = res_abc - 1
                    enc.resultat_visitant_sense_doble = res_xyz
                else:
                    enc.guanyador_doble = 2
                    enc.resultat_local_sense_doble = res_abc
                    enc.resultat_visitant_sense_doble = res_xyz - 1                   
            else:
                ...
                if fcd[lresabc[3]] == 3:
                    enc.guanyador_doble = 2
                    enc.resultat_local_sense_doble = res_xyz
                    enc.resultat_visitant_sense_doble = res_abc - 1                 
                else:
                    enc.guanyador_doble = 1
                    enc.resultat_local_sense_doble = res_xyz - 1
                    enc.resultat_visitant_sense_doble = res_abc                    
            enc.save()
            ...
            return render(request,'programa/acta.html',
                {'myform':myform,'local':enc.local,'visitant':enc.visitant,'competicio':comp_nom})
    else:
        myform = ActaForm(enc)
        response = render(request,'programa/acta.html',
                {'myform':myform,'local':enc.local,'visitant':enc.visitant,'competicio':comp_nom})
...
def ranquingtti(request):
    # Query to retrieve ranquingtti table    
    from django.db import connection, transaction
    cursor = connection.cursor()
    
    query=''' SELECT a.jugador, a.equip,
                    a.torneig1,a.torneig2,a.torneig3,a.torneig4,a.torneig5,
                    a.torneig6,a.torneig7,a.torneig8,a.torneig9,a.torneig10,
                    a.torneig11,a.torneig12,a.torneig13,a.torneig14,a.torneig15,
                    a.total
                FROM programa_ranquingtti a
                ORDER BY a.total DESC'''
                
    cursor.execute(query)
  
    tranquingtti = cursor.fetchall()    

    query=''' SELECT a.torneig
                FROM programa_tornejosranquingtti a
                ORDER BY a.id ASC'''
                
    cursor.execute(query)
  
    ttornejostti = cursor.fetchall()       
    
    return render(request,'programa/ranquingtti.html',
            {'ranquingtti':tranquingtti,'tornejostti':ttornejostti})
        
def estadistiques(request):
    return render(request,'programa/estadistiques.html',{})

def carregarfitxer(request):

    # Aquesta funcio no funciona. sha dajustar. sobretot poder llegir el fitxer amb accents i amb el format correcte. De moment penjaran el ranquingtti com a imatge.
    # Query to retrieve ranquingtti table
    ...
    url = 'http://intercomarcal.alwaysdata.net/site_media/ranquingtti.csv'
    response = urlopen(url)
    cr = csv.reader(response,delimiter=';')
    
    query="DROP table if exists programa_ranquingtti;"
    cursor.execute(query)
    query="Truncate table programa_tornejosranquingtti;"
    cursor.execute(query)
    ...
    for row in cr:
        ...
        for i in range (len(row)):
            rowtreated.append(row[i])
    
        queryList.append(rowtreated)

    query="Create table programa_ranquingtti ("
    
    for i in range(len(queryList[0])):
        query = query + "a" + str(i) + " varchar(100)"
        if i != len(queryList[0])-1:
            query = query + ","
    ...
    for i in range (len(queryList[0])):
        queryfields = queryfields + "a" + str(i)
        if i != len(queryList[0])-1:
    ...
    query = "INSERT INTO programa_ranquingtti (" + queryfields + ") VALUES (" + queryvalues + ");"
    
    #la primera fila son els noms dels tornejos i per aixo no linsertem
    
    cursor.executemany(query,queryList[1:len(queryList)])
    
    #insertem els tornejos a la taula de tornejos
    
    query = "INSERT INTO programa_tornejosranquingtti (Torneig) VALUES (%s);"    
    
    cursor.executemany(query,queryList[0])
    
    cursor.close     
    transaction.commit_unless_managed()
    
    return render(request,'programa/carregarranquing.html',{'bb':queryList[0]})